...
import com.junit.caozhiou.sideproject.view.viewpager.utils.JsonTool;
...
public abstract class BaseLoopView extends RelativeLayout implements ILoopView {
    ...
    public void setLoopViewPager(List<Map<String, String>> data) {
        ...
        for (Map<String, String> map : data) {
        	...
        	itemData.setImgUrl( map.get("imageURL"));
        	itemData.setLink( map.get("link"));
            mLoopData.getItems().add(itemData);
        }
    ...
    private void initLoopViewPager() {
        ...
        initDots(mLoopData.getItems().size());                     // 初始化指示点
       
        setViewListener();                                    // 初始化点击监听事件
        int startPosition = Integer.MAX_VALUE / 2 - Integer.MAX_VALUE / 2 % mLoopData.getItems().size();
    ...
    public void startAutoLoop(long delayTimeInMills) {
        if (null == mLoopData || mLoopData.getItems().size() <= 1) return;
        isAutoScroll = true;
