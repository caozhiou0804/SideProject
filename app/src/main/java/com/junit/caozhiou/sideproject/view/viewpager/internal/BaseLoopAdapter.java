...
import com.junit.caozhiou.sideproject.view.viewpager.utils.RecycleBitmap;
...
public abstract class BaseLoopAdapter extends PagerAdapter {
    ...
    @Override
    public int getCount() {
        if(mLoopData.getItems().size() <= 1){
            return mLoopData.getItems().size();
        }
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        final View view;
        final int index = position % mLoopData.getItems().size();
        String imageUrl = mLoopData.getItems().get(index).getImgUrl();
